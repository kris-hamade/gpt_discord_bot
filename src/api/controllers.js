...
exports.status = async (req, res) => {
    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Bot status requested.`);
    res.send('Bot is up and running');
};

// Get Bot Config /api/config
exports.config = async (req, res) => {
    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Bot config requested.`);
    res.send(getConfigInformation());
};

// Get Bot Uptime /api/uptime
exports.uptime = async (req, res) => {
    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Bot uptime requested.`);
    res.send(getUptime());
};
...
exports.clearChatHistory = (req, res) => {
    ...
    const chatHistoryPath = path.join(__dirname, '../utils/data-misc/chathistory.json');

    // Check if the file exists
    if (!fs.existsSync(chatHistoryPath)) {
        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Chat history file does not exist.`);
...
exports.clearUploadData = (req, res) => {
    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Clear upload data requested.`);
    ...
    fs.readdir(uploadsDirectory, (err, files) => {
        if (err) {
            ...
            return res.status(500).json({
                success: false,
                message: 'An error occurred while trying to read the uploads directory.'
            });
        }

        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Files to delete:`, files);
        ...
        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] All uploaded files have been successfully deleted.`);
        return res.json({
            success: true,
...
exports.getChatHistory = (req, res) => {
    ...
    const chatHistoryFilePath = path.join(__dirname, '../utils/data-misc/chathistory.json');
    ...
    try {
        ...
        const chatHistory = JSON.parse(fs.readFileSync(chatHistoryFilePath, 'utf-8'));
...
exports.listFiles = (req, res) => {
    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] List files requested.`);
    const type = req.params.type;
    const dataJsonDir = path.join(__dirname, '../utils/data-json');

    let files;
    if (type === 'All') {
        files = fs.readdirSync(dataJsonDir).filter(fn => fn.endsWith('Export.json')).sort().reverse();
    } else {
        files = fs.readdirSync(dataJsonDir).filter(fn => fn.endsWith(`${type}Export.json`)).sort().reverse();
    }

    console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] ${files.length} ${type} files found:`, files);
...
exports.uploadRoll20Data = async (req, res) => {
    ...
    if (serverFileName) {
        ...
        // Read server file
        serverData = JSON.parse(fs.readFileSync(originalFilePath, 'utf-8'));

        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Server file found:`, serverFileName);
        ...
        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Server file archived:`, archiveFilePath);
    }

    try {
        // Read uploaded file
        const uploadedData = JSON.parse(fs.readFileSync(uploadedFilePath, 'utf-8'));
        ...
        const newFilePath = path.join(dataJsonDir, `${moment().format('YYYYMMDD')}-${type}Export.json`);
        fs.writeFileSync(newFilePath, JSON.stringify(serverData, null, 2));

        console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] New file written:`, newFilePath);
